...
Les règles RTDB s'appliquent **depuis la console** : aucun workflow ne les déploie.
`firebase-database-rules.json` à la racine du dépôt est la copie de référence du ruleset
complet — ne JAMAIS coller un fichier partiel dans la console : cela écraserait
l'intégralité du ruleset (tours, registrations, community cassés en production).
...
2. **Vérifier la cascade `.read`** : en RTDB, une règle `.read: true` sur un ancêtre ne peut
   ...
   branche publique : `community-photos`, etc.) et laisser le défaut à `false`.
